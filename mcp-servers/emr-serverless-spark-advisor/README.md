...
```
MCP Client (Kiro/Claude/Q CLI)
        │ stdio
        ▼
  MCP Server (local)          ← spark_advisor_mcp.py (12 tools)
        │ SSH
        ▼
  EMR Primary Node            ← spark_extractor.py + emr_recommender.py
        │
        ▼
  S3 Event Logs               ← Spark event logs (any source)
```
